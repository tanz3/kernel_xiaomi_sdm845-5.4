...
static int sde_backlight_device_update_status(struct backlight_device *bd)
{
	...
	if (!dsi_display->panel->bl_config.bl_remap_flag) {
	...
	if (bl_lvl && bl_lvl < dsi_display->panel->bl_config.bl_min_level && !dsi_display->panel->bl_config.bl_remap_flag)
...
void sde_connector_helper_bridge_enable(struct drm_connector *connector)
{
	...
	if (!display->is_first_boot && !sde_in_trusted_vm(sde_kms) && c_conn->bl_device) {
